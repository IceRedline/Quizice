...
Справочник для разработчика и агента. Проверен по коду `main` на коммите `ebb95e3` (24.09.2026). Описывает реализацию iOS-клиента; серверного кода в репозитории нет, фактический production API в рамках этой проверки не вызывался. Контракты ниже — то, что отправляет и ожидает клиент. Значения секретов и адрес развёрнутого бэкенда здесь не хранятся.
...
- Если сервер прислал меньше вопросов, игра использует фактическое число. Пустой список показывает состояние недоступности/завершённого прогресса, а не результат `0/0`.
...
- Replay обычной темы пытается получить новую пачку с прежними количеством/сложностью. При ошибке coordinator запускает новый presenter с прежней темой в памяти.
- Есть особенность replay случайной подборки: сначала строится локальная подборка. Если её нельзя собрать (обычно у серверного каталога нет локальных вопросов), сразу переиспользуется текущая тема без нового запроса.
...
| UserDefaults, `quizice.statistics.attempts` и `.user.<encodedID>` | Гостевая и отдельные пользовательские статистики: baseline, pending attempts, migration ID, legacy summary; `.active-user` выбирает текущую |
...
Карточка статистики показывает число игр, долю верных ответов (общие верные / общие вопросы, округление до целого процента) и лучший результат. Лучший выбирается по доле правильных ответов, при равенстве — по числу правильных. Это агрегаты, не экран серверной истории всех игр.

**Прогресс ответов.** Событие создаётся только для вопроса с `questionID`, `questionVersion` и `locale`. Очередь отправляется при добавлении события, запуске приложения, активации сцены и восстановлении сети (`NWPathMonitor`). Удаляются только подтверждённые ID; ошибка оставляет очередь на диске. Очередь одна на установку, в событиях нет user ID: они отправляются с токеном, доступным на момент отправки. Локальные предпочтения тем тоже разделены по языку, но не по аккаунту. Это отличается от изолированных кэшей итоговой статистики.
