...
check_snapshot_coverage_markers() {
  ...
  require_fixed_string "$SCREEN_SNAPSHOT_TESTS" 'testRadarLongContentCompactQuestionSnapshot' 'Screen snapshots must cover long question and theme text on compact Radar layout'
  require_fixed_string "$SCREEN_SNAPSHOT_TESTS" 'testCleanLongQuestionCompactPortraitSnapshot' 'Screen snapshots must cover long questions on compact Clean layout'
  ...
  require_fixed_string "$CROSS_SCREEN_VISUAL_TESTS" 'testRadarLongQuestionShrinksWithoutTruncatingOnCompactPhone' 'Cross-screen visual tests must cover compact long-question fitting'
