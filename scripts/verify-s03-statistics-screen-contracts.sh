...
require_fixed_string "$THEMES_SERVICE" 'return CGSize(width: availableWidth, height: 112)' 'Statistics card must span the full collection row as a rectangular card'
require_fixed_string "$THEMES_SERVICE" 'let twoColumnWidth = floor((availableWidth - itemSpacing) / 2)' 'Theme cards must use a two-column vertical grid width calculation'
require_fixed_string "$QUIZ_VIEW_CONTROLLER" 'layout.scrollDirection = .vertical' 'Home collection must scroll vertically'
require_fixed_string "$QUIZ_VIEW_CONTROLLER" 'themesCollectionView.alwaysBounceVertical = true' 'Home collection must behave as a vertical scrolling grid'
require_fixed_string "$QUIZ_VIEW_CONTROLLER" 'themesCollectionView.bottomAnchor.constraint(equalTo: feelingLuckyButton.topAnchor, constant: -24)' 'Home collection must have a non-collapsing vertical height between the title and button stack'
...
require_fixed_string "$STATISTICS_VIEW_CONTROLLER" 'backButton.accessibilityIdentifier = "statisticsBackButton"' 'Statistics screen must expose an explicit back button when the navigation bar is hidden'
require_fixed_string "$STATISTICS_VIEW_CONTROLLER" 'navigationController.popViewController(animated: true)' 'Statistics back button must return to the home screen through navigation pop'
